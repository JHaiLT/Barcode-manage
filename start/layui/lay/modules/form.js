	...
	var d = function() {
			...
			return layui.each(v, function(e, t) {
				if(t.name = (t.name || "").replace(/^\s*|\s*&/, ""), t.name) {
					...
					if((/^checkbox|radio$/.test(t.type) && t.checked) || !/^checkbox|radio$/.test(t.type)) {
						if(u.hasOwnProperty(t.name)) {
							if(typeof u[t.name] != 'object' || u[t.name].constructor != Array) {
								var old = u[t.name];
								u[t.name] = new Array;
								u[t.name].push(old);
							};
							u[t.name].push(t.value)
						} else {
							u[t.name] = t.value
						}
					}
