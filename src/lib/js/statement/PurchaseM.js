...
$(document).ready(function(){
 ...
 function getNowFormatDate() {
					...
					success: function(result){									
						$.each(result.resultdata, function (index, item) {
							...
							optionStr += "<option value='" + item.ID + "'>"
									...
									$("#ProductionIDM").append(optionStr);
	...
	//查找
		$("#PurchaseM_query").click(function(){
			var Cdata=$("#Purchase_dateM").val();
			var Pid=$("#ProductionIDM").val();
			if(Cdata.toString()==0){
		     	layer.msg('请选择日期')
		     }else{
		...
		$("#bills_production_query3").click(function(){
			var Cdata=$("#Purchase_dateM").val();
			var Pid=$("#ProductionIDM").val();
			if(Cdata.toString()==0){
		     	layer.msg('请选择日期')
